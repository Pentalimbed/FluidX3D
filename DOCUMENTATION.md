...
### CMake
- Compile the default benchmark with:
  ```bash
  cmake -S . -B build
  cmake --build build --config Release
  ```
- Run the executable from `build/bin/Release/FluidX3D.exe` on Visual Studio multi-config builds, or `build/bin/FluidX3D` on single-config generators.
- To select a specific GPU, pass device IDs to the executable, for example `build/bin/FluidX3D 0` or `build/bin/FluidX3D 0 1 3 6` if the setup is [configured as multi-GPU](#the-lbm-class).
- The default configuration header is [`examples/benchmark/defines.hpp`](examples/benchmark/defines.hpp). To build with a downstream configuration header, point CMake at a directory containing your own `defines.hpp`:
  ```bash
  cmake -S . -B build -DFLUIDX3D_DEFINES_DIR=/path/to/config
  cmake --build build --config Release
  ```
- Use `-DFLUIDX3D_BUILD_EXAMPLES=OFF` to build only the backend and visualization library targets without the benchmark executable.
- Compiling requires CMake and a compiler with C++17 support. On Windows, install Visual Studio with the Desktop development with C++ workload.
- [`INTERACTIVE_GRAPHICS`](examples/benchmark/defines.hpp) needs desktop graphics support. On Linux/macOS, CMake links X11 when available.
- On macOS, [`INTERACTIVE_GRAPHICS`](examples/benchmark/defines.hpp) mode requires [XQuartz](https://www.xquartz.org/) to be installed.
- On Android, [`INTERACTIVE_GRAPHICS`](examples/benchmark/defines.hpp) mode is not supported, as no X11 is available. You can still use [`INTERACTIVE_GRAPHICS_ASCII`](examples/benchmark/defines.hpp) though, or [render video](#video-rendering) to the hard drive with regular [`GRAPHICS`](examples/benchmark/defines.hpp) mode.

<br>

## 3. Go through Sample Setups
- Now open [`examples/benchmark/setup.cpp`](examples/benchmark/setup.cpp). In here are all the sample setups, each one being a `void main_setup() {...}` function block written in C++. Uncomment one of them, maybe start top-to-bottom.
- In the line where the `main_setup()` function starts, it says "required extensions in defines.hpp:", followed by a list of extensions in capital letters. Head over to [`examples/benchmark/defines.hpp`](examples/benchmark/defines.hpp) and comment out
  ```c
  //#define BENCHMARK
  ```
  with a `//`. Then, uncomment all of the extensions required for the setup by removing the `//` in front of the corresponding line.
- Finally, [compile](#2-compiling-the-source-code) and run the rebuilt executable.
...
## 4. Keyboard/Mouse Controls for [`INTERACTIVE_GRAPHICS`](examples/benchmark/defines.hpp)/[`_ASCII`](examples/benchmark/defines.hpp)
| Key                       | Function                                                                                                                           |
| :-----------------------: | :--------------------------------------------------------------------------------------------------------------------------------- |
|                           |                                                                                                                                    |
...
- A good beginner example for this is the "[aerodynamics of a cow](examples/benchmark/setup.cpp)" setup.
...
- Available Boundary Conditions
  ...
  - Equilibrium Boundaries (`TYPE_E` with specified density and/or velocity)
    - You need to enable (uncomment) the [`EQUILIBRIUM_BOUNDARIES`](examples/benchmark/defines.hpp) extension.
  ...
  - Moving Solid Boundaries (`TYPE_S` with non-zero velocity)
    - You need to enable (uncomment) the [`MOVING_BOUNDARIES`](examples/benchmark/defines.hpp) extension.
...
- Rotating geometries have to be periodically revoxelized, about every 1-10 LBM time steps. In the main simulation loop in the [`main_setup()`](examples/benchmark/setup.cpp) function, first rotate the triangle mesh, then revoxelize on GPU, then compute a few LBM time steps:
  ```c
...
- Have a look at the "[Cessna 172](examples/benchmark/setup.cpp)" and "[Bell 222](examples/benchmark/setup.cpp)" setups for some examples.

### Video Rendering
- For video rendering, disable (comment out) [`INTERACTIVE_GRAPHICS`](examples/benchmark/defines.hpp) and [`INTERACTIVE_GRAPHICS_ASCII`](examples/benchmark/defines.hpp) and enable (uncomment) [`GRAPHICS`](examples/benchmark/defines.hpp) in [`examples/benchmark/defines.hpp`](examples/benchmark/defines.hpp).
- Set the video resolution as [`GRAPHICS_FRAME_WIDTH`](examples/benchmark/defines.hpp)/[`GRAPHICS_FRAME_HEIGHT`](examples/benchmark/defines.hpp) and the background color as [`GRAPHICS_BACKGROUND_COLOR`](examples/benchmark/defines.hpp). You can also adjust the other [`GRAPHICS_...`](examples/benchmark/defines.hpp) options there, such as semi-transparent rendering mode, or adjust the color scale for velocity with [`GRAPHICS_U_MAX`](examples/benchmark/defines.hpp).
- A basic loop for rendering video in the [`main_setup()`](examples/benchmark/setup.cpp) function looks like this:
  ```c
  ...
  ```
- To find suitable camera placement, run the simulation at low resolution in [`INTERACTIVE_GRAPHICS`](examples/benchmark/defines.hpp) mode, rotate/move the camera to the desired position, click the <kbd>Mouse</kbd> to disable mouse rotation, and press <kbd>G</kbd> to print the current camera settings as a copy-paste command in the console. <kbd>Alt</kbd>+<kbd>Tab</kbd> to the console and copy the camera placement command by selecting it with the mouse and right-clicking, then paste it into the [`main_setup()`](examples/benchmark/setup.cpp) function.
...
### Lift/Drag Forces
- Enable (uncomment) the [`FORCE_FIELD`](examples/benchmark/defines.hpp) extension. This extension allows computing boundary forces on every solid cell (`TYPE_S`) individually, as well as placing an individual volume force on every fluid cell (not used here).
- In the [`main_setup()`](examples/benchmark/setup.cpp) function, voxelize the mesh with a unique flag combination, such as `(TYPE_S|TYPE_X)` or `(TYPE_S|TYPE_Y)` or `(TYPE_S|TYPE_X|TYPE_Y)`, to distinguish it from all other `(TYPE_S)` cells that might be needed to define other geometry, and compute its center of mass:
  ```c
  ...
  ```
- To sum over all the individual boundary cells that belong to the object, in the [`main_setup()`](examples/benchmark/setup.cpp) function's main simulation loop call:
  ```c
...
- Finally, [convert from LBM to SI units](#unit-conversion) with
  ...
  ```
  after having done [unit conversion](#unit-conversion) with `units.set_m_kg_s(...)`.
- See the "[Ahmed body](examples/benchmark/setup.cpp)" sample setup for an example. Note that in the highly turbulent regime, computed body forces are too large by up to a factor 2, because even large resolution is not enough to fully capture the turbulent boundary layer. A wall function is still needed.

<br>

## 6. Further LBM Extensions
By now you're already familiar with the [additional boundary types](#initial-and-boundary-conditions) through extensions [`VOLUME_FORCE`](examples/benchmark/defines.hpp), [`FORCE_FIELD`](examples/benchmark/defines.hpp), [`EQUILIBRIUM_BOUNDARIES`](examples/benchmark/defines.hpp), and [`MOVING_BOUNDARIES`](examples/benchmark/defines.hpp). The remaining available model extensions are briefly outlined here:

### [`SURFACE`](examples/benchmark/defines.hpp) Extension
- To simulate free water surfaces, enable (uncomment) the [`SURFACE`](examples/benchmark/defines.hpp) extension.
...
- See the "[hydraulic jump](examples/benchmark/setup.cpp)" or the "[raindrop impact](examples/benchmark/setup.cpp)" sample setups.

### [`TEMPERATURE`](examples/benchmark/defines.hpp) Extension
- With the [`TEMPERATURE`](examples/benchmark/defines.hpp) extension, FluidX3D can model thermal convection flows. This extension automatically also enables the [`VOLUME_FORCE`](examples/benchmark/defines.hpp) extension.
...
- To set temperature boundary conditions, use the flag `TYPE_T` and for the same cells assign a temperature unequal to `1`:
  ...
  ```
- See the "[Rayleigh-Benard convection](examples/benchmark/setup.cpp)" and "[thermal convection](examples/benchmark/setup.cpp)" setups for two examples.

### [`SUBGRID`](examples/benchmark/defines.hpp) Extension
...
- The subgrid model in FLuidX3D is the Smagorinsky-Lilly model. You can enable it with the [`SUBGRID`](examples/benchmark/defines.hpp) extension.
- There is no additional performance cost for this extension.

### [`PARTICLES`](examples/benchmark/defines.hpp) Extension
- By default, the LBM is a grid-based simulation, so there are no particles.
- But the [`PARTICLES`](examples/benchmark/defines.hpp) extension allows to add particles to the simulation, either as passive tracers or as 2-way-coupled particles that can do floating/sedimentation.
- For passive tracers, only enable the [`PARTICLES`](examples/benchmark/defines.hpp) extension, and in the [LBM constructor](#the-lbm-class) simply add the particle count:
  ```c
...
- For 2-way-coupled particles, additionally enable the [`VOLUME_FORCE`](examples/benchmark/defines.hpp) and [`FORCE_FIELD`](examples/benchmark/defines.hpp) extensions, and in the [LBM constructor](#the-lbm-class) add the particle density ([in LBM units](#unit-conversion)) unequal to `1`:
  ```c
...
- Often times, the cause of instability is an unfortunate choice of unsuitable parameters:
  ...
  - too low kinematic shear viscosity `nu` (ideally close to `1/6`, becomes unstable when it's very very close to `0` (then enable the [`SUBGRID`](examples/benchmark/defines.hpp) extension), and should not exceed `3`)
